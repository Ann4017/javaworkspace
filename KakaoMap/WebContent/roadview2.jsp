...
<script>
	function select_road_view(_this) {
		...
		var pharmacy_name = _this.previousElementSibling.textContent;
		...
		geocoder.addressSearch(addr, function(result, status) {
			...
			if (status === kakao.maps.services.Status.OK) {
				...
				mapCenter = new kakao.maps.LatLng(result[0].y, result[0].x), // 지도의 중심 좌표
				mapOption = {
				...
				var mLabel = new kakao.maps.InfoWindow({
					position : mapCenter, // 지도의 중심좌표에 올립니다.
					content : pharmacy_name // 인포윈도우 내부에 들어갈 컨텐츠 입니다.
				});
	...
	$.getJSON(url, function(data) {
		for (let i = 0; i < data.currentCount; i++) {
			let row = "<tr>" + "<td onclick='get_pharmacy_name(this)'>"
					+ data.data[i].약국명칭 + "</td>"
	...
	<div class="wrap_content">
		<div class="wrap_map">
			...
			<div class="wrap_button">
				<a href="javascript:;" class="btn_comm btn_linkMap" target="_blank"
					onclick="moveKakaoMap(this)"><span class="screen_out">지도
						크게보기</span></a>
				<!-- 지도 크게보기 버튼입니다 -->
